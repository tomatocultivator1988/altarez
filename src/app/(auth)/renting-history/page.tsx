...
export const metadata: Metadata = {
  title: "Renting History | Agrimalachina",
  description: "View your completed and past machinery rentals.",
}
...
export default async function RentingHistoryPage() {
  ...
  const { data: bookings } = await supabase
    .from("bookings")
    .select("*, machinery(machine_name, machine_type), owner:profiles!bookings_owner_id_fkey(first_name, last_name)")
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">Renting History</h1>
      {!bookings?.length ? (
              ...
              <Card key={b.id}>
                <CardContent className="flex items-center justify-between p-4">
                  <div>
                    ...
                    {owner && <p className="text-xs text-muted-foreground">Owner: {owner.first_name}</p>}
                  </div>
